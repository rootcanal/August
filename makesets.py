...
def assert_question_entity(story, sets, good):
    ...
    if xset and good == 0:
        ...
        quantifiedents = [x[1].entity for x in sets
                          if floatcheck(x[1].num) or x[1].num == 'dozen']
        if quantifiedents:
...
def question_entity(story, sets):
    ...
    if "what" in [x[0].lower() for x in words]:
        targets = [x[2] for x in deps
                   if 'what' in x[1].lower() and x[0] == 'nsubj']
        if len(targets) == 1:
            t, tidx = targets[0].rsplit("-", maxsplit=1)
            tidx = int(tidx) - 1
            lemma = words[tidx][1]["Lemma"]
            sets.append(
                (j * 1000 + tidx, aset('x', lemma, t, j * 1000 + tidx))
            )
    if "how" in [x[0].lower() for x in words]:
        targets = [x[1] for x in deps
                   if x[2].rsplit("-", maxsplit=1)[0] in ['many', 'much']]
        ...
        if 'much' in wzeros:
            if ('cost' in wzeros) or ('spend' in wzeros):
                ...
                sets.append(
                    (j * 1000 + tidx,
                     aset('x', 'dollar', "dollar", j * 1000 + tidx))
                )
                good = 1
                targets = []
        ...
        if len(targets) == 1:
            ...
            if words[tidx][1]["PartOfSpeech"] in ["NN", "NNS"]:
                lemma = words[tidx][1]["Lemma"]
                #check for dozen
                if len([x for x in deps
                        if targets[0] == x[1]
                        and x[0] == 'nn' and 'dozen' in x[2]]) > 0:
                    sets.append(
                        (j * 1000 + tidx - 1,
                         aset('x', 'dozen', 'dozen', j * 1000 + tidx))
                    )
                    sets.append(
                        (j * 1000 + tidx,
                         aset('dozen', lemma, t, j * 1000 + tidx))
                    )
                else:
                    sets.append(
                        (j * 1000 + tidx, aset('x', lemma, t, j * 1000 + tidx))
                ...
                if t == "more":
                    targets = [x[1] for x in deps
                               if x[2].rsplit("-", maxsplit=1)[0] in ['more']]
                    if targets:
                        t, tidx = targets[0].rsplit("-", maxsplit=1)
                        tidx = int(tidx) - 1
                        if words[tidx][1]["PartOfSpeech"] in ["NN", "NNS"]:
                            ...
                            sets.append(
                                (j * 1000 + tidx,
                                 aset('x', lemma, t, j * 1000 + tidx))
                            )
                            good = 1
                elif t == 'did':
                    targets = [x[2] for x in deps
                               if x[1].rsplit("-", maxsplit=1)[0] in ['did']
                               and x[0] == 'nsubj']
                    if targets:
                        t, tidx = targets[0].rsplit("-", maxsplit=1)
                        tidx = int(tidx) - 1
                ...
                if good == 0:
                    sets.append(
                        (j * 1000 + tidx,
                         aset('x', 'NONE', 'NONE', j * 1000 + tidx))
                    )
                    #good = 1
        else:
            howidx = [i for i, x in enumerate(words)
                      if x[0].lower() == 'how'][0]
            nextword = words[howidx + 1]
            if nextword[0] == 'far':
                sets.append(
                    (j * 1000 + howidx + 1,
                     aset('x', 'DISTANCE', 'DISTANCE', j * 1000 + howidx + 1))
                )
                good = 1
            elif nextword[0] == 'long':
                sets.append(
                    (j * 1000 + howidx + 1,
                     aset('x', 'LENGTH', 'LENGTH', j * 1000 + howidx + 1))
                )
                good = 1
...
def extract_quantify(story):
    ...
    for j, s in enumerate(story):
        ...
        nums = [(x[1], x[2]) for x in deps
                if x[0] == 'num' or x[0] == 'number' or x[0] == 'det']
        nums.extend([(x[2], x[1]) for x in deps
                     if x[0] == "prep_of"
                     and (x[1].split("-")[0].isdigit()
                          or x[1].rsplit("-", maxsplit=1)[0]
                          in ['half', 'third', 'quarter'])])
        ...
        for w, n in nums:
            ...
            if w == "$":
                # standardized representation of collection of forms of word
                lemma = 'dollar'
                sets.append(
                    ((j * 1000) + nidx, aset(n, lemma, w, j * 1000 + widx))
                )
                ...
                sets.append(
                    ((j * 1000) + nidx, aset(n, lemma, w, j * 1000 + widx)))
                if [x for x in deps
                        if x[0] == 'prep_of' and x[1] == n and x[2] == w]:
                    sets[-1][1].subset = 1
...
def bad_parse_each(sets, words, j):
    ...
    if 'each' in surfaces:
        ...
        if (j * 1000) + eachi in [x[0] for x in sets]:
            return sets
        setmatch = [x for x in words[eachi:eachi + 4]
                    if x[1]['Lemma'] in [y[1].entity for y in sets]]
        if setmatch and len([x for x in words[eachi:eachi + 4]
                             if x[0] in [',', 'and', 'but']]) == 0:
            nextword = setmatch[0]
        else:
            setmatch = [x for x in words
                        if x[1]['Lemma'] in [y[1].entity for y in sets]]
            if setmatch:
                nextword = setmatch[0]
            else:
        ...
        sets.append(
            ((j * 1000) + eachi,
             aset('each', lemma, nextword[0], j * 1000 + eachi + 1))
        )
    ...
    if dozenents:
        for x in dozenents:
            if thisdozen:
                ...
                if thisdozen[0][1].num != 'x':
                    if not floatcheck(thisdozen[0][1].num):
                        thisdozen[0][1].num = '1'
            else:
        ...
        if thisdozen:
            dozdeps = [x[2].split('-') for x in deps
                       if 'dozen' in x[1] and 'many' not in x[0]]
            if dozdeps:
                #print(dozdeps)
                wrds = [s['words'][int(x[1]) - 1] for x in dozdeps]
                wrds = [x for x in wrds
                        if x[1]['PartOfSpeech'] in ['NN', 'NNS']]
                if wrds:
                    wrd = wrds[0]
                    lem = wrd[1]['Lemma']
                    surf = wrd[0]
                    dozent = aset(
                        '12', lem, surf, j * 1000 + thisdozen[0][0] + 1
                    )
...
def containers_each(sets, deps, thissentsets, j, s):
    #deal with each
    eaches = ['each', 'a', 'an', 'the', 'every', 'per', 'one', '1']
    thiseach = [x for x in thissentsets if x[1].num in eaches]
    if len(thiseach) > 0:
        ...
        if thisothers:
            for eidx, e in thiseach:
                ...
                if e.num in ['a', 'an', 'per']:
                    prev = [x for x in thisothers
                            if x[0] < eidx and x[0] > eidx - 5]
                    if prev:
                        target = prev[-1]
                    ...
                    for x in deps:
                        ...
                        for y in dep:
                            i = int(y.split('-')[-1]) - 1
                            meachdeps.append(s['words'][i][1]['Lemma'])
                    #e.details()
                    #print(eachdeps)
                    eachdeps = [y for y in sets
                                if floatcheck(y[1].num) and
                                y[1].entity in meachdeps and
                                y[1].surface + "-" + str(y[1].widx) in
                                meachdeps]
                    #print(eachdeps,eidx)
                    if eachdeps:
                        try:
                            e = [(abs(y[0] - eidx), y[1]) for y in eachdeps]
                            each0 = sorted(e, reverse=True)[0]
                        except:
                            each0 = eachdeps[0]
...
def articulate(sets, deps, thissentsets, j, s):
    for e in thissentsets:
        ...
        # location
        elocs = [x[2].split("-")[0] for x in deps
                 if x[0] in ['prep_in', 'prep_on', 'prep_at']
                 and x[1] == esurface]
        if elocs:
        ...
        if vbs:
            for y in vbs:
                ...
                vsubj = [x for x in deps
                         if x[1] == verb and x[0] in ['nsubj', 'nsubjpass']]
                if vsubj:
                    e[1].container = ' '.join(
                        [x[2].split('-')[0] for x in vsubj]
                    )
        else:
            possible_containers = [x[1].container for x in sets
                                   if x[1].container is not None
                                   and x.lower()
                                   in [y[0].lower for y in s['words']]]
            if possible_containers:
                e[1].container = ' '.join(possible_containers)
        if not e[1].verbs:
            vbs = [x[1]["Lemma"] for x in s['words']
                   if 'VB' in x[1]['PartOfSpeech']]
...
def fix_each(sets):
    ...
    eaches = ['each', 'every', 'per', 'a', 'an', 'per', 'one']
    eaches = [x for x in sets if x[1].num in eaches]
    for x in eaches:
        moveto = [y for y in sets
                  if y[1].entity == x[1].entity and floatcheck(y[1].num)]
        if moveto:
            moveto = moveto[0]
            if x[0] > moveto[0]:
    ...
    while i < len(sets):
        if sets[i][1].container:
            containers = [x for x in sets
                          if x[1].entity == sets[i][1].container
                          and x[1].contains == sets[i][1].entity
                          and floatcheck(x[1].num)]
            if containers:
...
def circumscription(story, sets):
    for j, s in enumerate(story):
        words = s['words']
        othernums = [((j * 1000 + i), x[0]) for i, x in enumerate(words)
                     if x[1]["PartOfSpeech"] == "CD"]
        ...
        if othernums:
            for idx, n in othernums:
                ...
                if 'and' in [x[0] for x in words[idx % 1000:idx % 1000 + 7]]:
                    #use next jawn
                    nextjawn = [x for x in sets
                                if x[0] > idx
                                and x[0] < (((idx // 1000) + 1) * 1000)]
                    if nextjawn:
                        nextjawn = nextjawn[0][1]
                        entity = nextjawn.entity
                        surface = nextjawn.surface
                        sets.append(
                            (idx,
                             aset(n, entity, surface, j * 1000 + idx))
                        )
                        prev = 0
                if prev == 1:
                    ...
                    if prevjawn:
                        ...
                        entity = prevjawn.entity
                        surface = prevjawn.surface
                        sets.append(
                            (idx,
                             aset(n, entity, surface, j * 1000 + idx))
                        )
                        ...
                        for j, s in enumerate(story):
                            for i, w in enumerate(s['words']):
                                if w[1]["PartOfSpeech"] == "NNS":
                                    nns.append(j * 1000 + i, w)
                        #print(nns)
                        if nns:
                            prev = [x[1] for x in nns if x[0] < idx]
                            if prev:
                                prevjawn = prev[-1]
                                prevl = prevjawn[1]["Lemma"]
                                prevone = prevjawn[0]
                                sets.append(
                                    (idx,
                                     aset(n, prevl, prevone, j * 1000 + idx))
                                )
                            else:
                                prevjawn = nns[0][1]
                                prevl = prevjawn[1]["Lemma"]
...
def add_bare_sets(sets, story):
    quantifiedents = [x[1].entity for x in sets
                      if floatcheck(x[1].num) or x[1].num in ['dozen', 'half']]
    ...
    for j, s in enumerate(story):
        ...
        for i, w in enumerate(s['words']):
            ...
            if w[1]['Lemma'] in quantifiedents:
                sets.append(
                    ((j * 1000) + i,
                     aset('BARE', w[1]['Lemma'], w[0], j * 1000 + i))
                )
...
def fix_times(sets):
    ...
    if times:
        for x in times:
            pcontainer = [y[1].container for y in sets
                          if y[0] < x[0] and y[1].container is not None]
            if pcontainer:
...
def move_x(sets, story):
    ...
    for j, s in enumerate(story):
        thissentsets = [x for x in sets if x[0] // 1000 == j]
        potentials = [(i, x) for i, x in enumerate(s['words'])
                      if x[1]["Lemma"] == target]
        for i, p in potentials:
            smatch = [x for x in thissentsets
                      if x[1].surface == p[0] and x[1].widx == i + 1]
            if smatch:
                smatch = [x for x in smatch
                          if x[1].num != 'x' and not floatcheck(x[1].num)]
                spots.extend([(x[0], x[1].num) for x in smatch])
            else:
                #assure this is the only set ref in the vicinity
                closesets = [x for x in sets
                             if abs(x[0] - (j * 1000 + i)) < 10]
                if len(closesets) == 0:
...
def coref(sets):
    pros = [x for x in sets if x[1].container is not None]
    pros = [x for x in pros
            if x[1].container.lower() in ['he', 'she', 'her', 'his', 'him']]
    for pro in pros:
        others = [x for x in sets if x[1].container is not None]
        others = [x[1].container for x in others
                  if x[1].container[0].isupper() and x[0] < pro[0]]
        if others:
...
def prune(sets):
    ...
    if len(x) > 0:
        x = x[0]
        if x.adjs is not None:
            sets = [y for y in sets if y[1].adjs is not None]
